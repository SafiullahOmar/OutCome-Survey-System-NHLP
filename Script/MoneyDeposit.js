function CreateFarmerMoneyDepositTable(Year) {
    var obj = JSON.stringify({ LeadFormerId: $('#ddlLeadFarmer').val(), Year: Year });
    $.ajax({
        type: "POST",
...
function SaveForm(Year) {
    var formDetail = {};
    formDetail.FormerLeadId = $('#ddlLeadFarmer').val();
    formDetail.LeadFormerName = $('#ddlLeadFarmer').find('option:selected').text();
    ...
    $('#tblMoneyDeposit tbody tr').each(function () {
        var detail = {};
        if (detail.RMoney = $(this).find("input[id^='txtMoney']").val() != '')
            detail.RMoney = $(this).find("input[id^='txtMoney']").val();
        else
            detail.RMoney = "0";
...
function formValidation(e) {
    ...
    if ($('#ddlLeadFarmer').val() == '-1' || $('#ddlLeadFarmer').find('option:selected').text() == '') {
        $('#ddlLeadFarmer').css('background-color', '#FFAAAA');
        flag = false;
    }
    else { $('#ddlLeadFarmer').css('background-color', '#ffff'); }
...
function EditFormerAmountDetail(obj) {
    ...
    var obj = JSON.stringify({ FormerId: FormerId, Year: $('.year').val(), LeadFarmerId: $('#ddlLeadFarmer').val() });
    $.ajax({
        type: "POST",
...
function UpdateFormerAmountDetail() {
    var fDetail = {};
    fDetail.LeadFarmerId = $('#ddlLeadFarmer').val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.alert({
                ...
                closeIcon: true,
                title: 'Success Alert',
                content: 'Record has been successfully updated into the system!'
            });
            
            //Clear components
            $("#tblAmounEdit tbody tr").remove();
            CreateFarmerMoneyDepositTable( $('#hdnYear').val());
        },
        error: function (data) {
            alert("error found");
        }
